...
## Init the router with lab network setup
...
```bash
  ${SSH} root@${EDGE_ROUTER} "mkdir -p /data/var/named/data ; \
    cp -r /etc/bind /data/bind ; \
    mkdir -p /data/tftpboot/ipxe ; \
    mkdir /data/tftpboot/networkboot"
  ${SCP} ${OKD_LAB_PATH}/boot-files/ipxe.efi root@${EDGE_ROUTER}:/data/tftpboot/ipxe.efi
  ${SCP} ${WORK_DIR}/boot.ipxe root@${EDGE_ROUTER}:/data/tftpboot/boot.ipxe
  ${SCP} -r ${WORK_DIR}/dns/* root@${EDGE_ROUTER}:/data/bind/
  ${SSH} root@${EDGE_ROUTER} "mkdir -p /data/var/named/dynamic ; \
